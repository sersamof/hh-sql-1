...
create table users
(
  user_id    serial primary key,                  -- впрочем, если login неизменяем, то первичным ключем может быть и он
  ...
  registered timestamp    not null default now(), -- храним в UTC, конвертация таймзоны на слое представления
...
(
  ...
  admin       int           not null references users (user_id)
);

create table companies_hr
(
...
(
  ...
  expired     timestamp     not null -- можно default now() + 1 month, например, но такую логику лучше на слой приложения
);
...
(
  ...
  salary_from int,
  salary_to   int,
  public      boolean   not null default true -- для упрощения видимость только всем / никому
);
